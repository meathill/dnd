import { attributeOptions, type FormState, type SkillOption } from "./character-creator-data";
				...
				<div>
					...
					<div className="mt-2 flex flex-wrap gap-2">
						...
						{formState.buffs.map((buff) => (
							<span className="rounded-lg bg-[rgba(61,82,56,0.16)] px-3 py-1 text-xs text-[var(--accent-moss)]" key={buff}>
								{buff}
							</span>
						))}
						{formState.debuffs.map((debuff) => (
							<span className="rounded-lg bg-[rgba(176,74,53,0.16)] px-3 py-1 text-xs text-[var(--accent-ember)]" key={debuff}>
								{debuff}
							</span>
						))}
					</div>
