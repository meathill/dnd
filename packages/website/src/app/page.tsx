  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        ...
        <p className="max-w-2xl text-zinc-600">登录后可查看模组列表，选择人物卡，然后交由 opencode 与现成 skills 主持游戏。</p>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {modules.map((moduleRecord) => (
          <Card className="space-y-4" key={moduleRecord.id}>
            ...
            <Link className="text-sm font-medium text-zinc-950 underline underline-offset-4" href={`/modules/${moduleRecord.id}`}>
              查看摘要并开始
            </Link>
          </Card>
        ))}
